...
void QueryProcessor1::graphExp(Node &q, string graphType, PrimeSim &sim)
{
  ...
  while (expansion > 0)
  {
    ...
    for (auto &hid : borderHubsScore)
    {
      ...
      nextSim_hubsnum += nextSim.numHubs();
    ...
    nextSim_hubsnum = nextSim_hubsnum / (double) maxnode;
    string info = "eta: " + to_string((int)Config::eta) + ", expansion: " + to_string((int)expansion);
    Config::exp_eta_nums[info]++;
...
// void QueryProcessor1::query(int k, vector<nodePairVal> &result)
{
      ...
      for (int idx1 = 0; idx1 < endingNodes.size() - 1; idx1++)
      {
        for (int idx2 = idx1 + 1; idx2 < endingNodes.size(); idx2++)
        {
          ...
          if (Config::correctionLevel > 0)
          {
            ...
            if (n_in_degree > 0 && length.first < Config::depth)
            {
