...
int main(int argc, char **argv) {
	...
	string nodeset_path = "../gt/" + filename;
	myrbs.nodeset_init(nodeset_path);
	...
	topk_ans = myrbs.get_topk_medium(topk, eps_init);
	mytime.end();
	...
	mkdir_p("../result/" + filename + "/");

	// std::stringstream ss;
	// ss << setprecision(15) << d;
	// str = ss.str();
	string outputfile = "../result/" + filename + "/topk_" + stopk;
