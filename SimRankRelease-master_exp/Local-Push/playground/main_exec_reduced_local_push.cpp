...
int main(int argc, char *argv[]) {
    ...
    double eps = 0.1;
    ifstream idxin;
    string idx_path = "/data/geQian/packing_gq/rbs-sim_exp_yuetian/gt/" + g_name;
    ...
    cout << "nodeseta.size = " << nodeseta.size() << endl;
    ...
    while(true){
        ...
        // if(ve.size() >= topk){
        if (eps < 5e-3) {
            cout << "Stop !!! next _turn eps = " << eps << ", smaller than 5e-4 !!!" << endl;
            cout << "<" << rou << endl;
    ...
    if(ve.size() < topk){
        cout << "old ve.size = " << ve.size() << " smaller than topk = " << topk << ", reinsert ve!!!" << endl;
        ve.clear();
        for(auto i : nodeseta){
            auto tmp = lp->P.v[i];
            for(const auto j_pair:tmp){
