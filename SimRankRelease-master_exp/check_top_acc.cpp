...
int main(int argc, char **argv) {
	...
	string groundtruth_path = "/data/geQian/packing_gq/large_graph_gt/" + filename+"top"+stopk;
	...
	anspath = base_path + "Local-Push/topk_result/" + filename + ".ans" + stopk;
	...
	vector<Sim_pair> vec_my_sim_topk;
	if(vec_my_sim_topk.size() > topk){
		for(int i = 0; i < topk; ++i){
			vec_my_sim_topk.emplace_back(vec_my_sim[i]);
		}
	}
	else
		vec_my_sim_topk = vec_my_sim;
		// (vec_my_sim.begin(), vec_my_sim.begin() + topk);
	my_sim.clear();
	int larger_0num = 0;
	...
	int my_sim_pair_num = 0;
	for(auto i: my_sim){
		for(auto j: i.second)
			++my_sim_pair_num;
	}
	int true_num = 0;
	cout << "my ans larger than 0 num = " << larger_0num << endl;
	cout << "mysim_pair_num = " << my_sim_pair_num << endl;
	for (auto i : my_sim) {
		a = i.first;
		for (auto j : i.second) {
			b = j.first;
