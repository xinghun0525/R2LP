...
public:
    ...
    void get_ground_truth(string filename, int topk){
        ...
        filename == "DB" || filename == "WF" || filename == "ND") {
            ...
            // int gtnum = 0;
            string groundtruth_path = "/data/geQian/packing_gq/large_graph_gt/" + filename + "top" + to_string(topk);
            // for(auto a: ground_idx){
            //     string gtpath = groundtruth_path + to_string(a) + ".txt";
                    // cout << gtpath << endl;
            cout << groundtruth_path << endl;
                fin.open(groundtruth_path.data());
                assert(fin.is_open());
                ...
                fin.close();
            // }
            cout << "ground_topk.size = " << ground_topk.size() << endl;
        }
        return;
    }
    ...
    bool check_finish_large(double tmp_eps){
        vector<Sim_pair> ve;
        ve.clear();
        cout << "file name = " << file_name << endl;
        for (auto u1: ppr) {
            if(nodeseta.find(u1.first) != nodeseta.end()){
                for(auto u2: u1.second){
                    Sim_pair sp;
                    sp.a = u1.first; sp.b = u2.first; sp.sim = u2.second;
                    ve.emplace_back(sp);
                }
            }
            else{
                for(auto b: u1.second){
                    if(nodeseta.find(b.first) != nodeseta.end()){
                        Sim_pair sp;
                        sp.a = u1.first; sp.b = b.first; sp.sim = b.second;
                        ve.emplace_back(sp);
                    }
                }
            }
